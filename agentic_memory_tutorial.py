    ...
    return [
        {
            ...
            "score": r.score,
            **{k: v for k, v in r.payload.items() if k not in ["content", "summary", "timestamp", "user_id"]}
        }
        for r in results
    ]
...
def semantic_memory_search(
    ...
    return [
        {
            "knowledge": r.payload["knowledge"],
            "type": r.payload["knowledge_type"],
            "confidence": r.payload["confidence"],
            "score": r.score
        }
        for r in results
        if r.payload["confidence"] >= min_confidence
    ]
